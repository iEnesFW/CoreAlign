...
- **Optimistic concurrency = İKİ mekanizma, bilinçli seçilir (HYBRID — bunu bozma).** Çakışma her ikisinde de `DbUpdateConcurrencyException` → **409 `DomainConcurrencyException`** (`ConcurrencyTokenBehavior` ile bağlı), sessiz overwrite yok. Yeni yarışabilen entity'de **birini** seç (ikisini aynı entity'ye takma); mevcut seçimi gerekçesiz değiştirme:
  - **`IXminConcurrency`** → Postgres `xmin` sistem kolonu. **⚠️ PG18'DE DEVRE DIŞI (2026-07): `CoreAlignDbContext.ApplyXminConcurrencyTokens` bilinçli NO-OP** (`_ = modelBuilder;`) — PG18 `RETURNING xmin`'i reddediyor (SQLSTATE `0A000`) ve tüm finansal entity save'lerini kırıyordu. `IXminConcurrency` artık **ÖLÜ MARKER** (yalnız 2 yorum referansı; hiçbir entity taşımıyor — Phase119 ile tüm finansallar `IHasConcurrencyToken`'a taşındı). **Yeni yarışabilen finansal entity'de `IXminConcurrency` KULLANMA → `IHasConcurrencyToken`.**
  - **`IHasConcurrencyToken`** → app-managed `long ConcurrencyToken` + açık `BumpConcurrencyToken()` (SaveChangesAsync Modified'da otomatik bump'lar). **Hem Npgsql hem SQLite test provider'ında çalışır — PG18'de ÇALIŞAN tek mekanizma.** Kullananlar: `ProductVariant, StockItem (Phase71), VendorBill/VendorPayment/PurchaseOrder (Phase117), Invoice/Order/Payment/JournalEntry/CustomerLedgerEntry/VendorLedgerEntry/Employee/PayrollRun/Payslip (Phase119), Shipment (Phase120), FxRate, DataSubjectRequest, GlassProject ailesi (FieldSurvey/GlassProject/GlassProjectPanel/GlassProjectRun/GlassWorkOrder)`.
