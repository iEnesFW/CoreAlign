...
| **Bildirim · Provider · Outbox · E-Fatura**                    | `Application/{Notifications,Providers,EInvoice}`, `*Outbox*`                                              | Outbox pattern; tenant-aware SMTP (`TenantProviderConfig`); doküman-forward IDOR-safe; provider creds prod'da sandbox-only                                                                               | `docs/modules/notification-service.md`; `.Codex/memory/project_corealign_notification_integration.md` |
...
CoreAlign'a özel Codex varlıkları `.Codex/` altında, **repoyla taşınır** (detay: `.Codex/README.md`).

### 18.1 Otomatik yüklenen bağlam

- **`AGENTS.md`** (bu dosya) + **`@docs/INVARIANTS.md`** (üstteki import) her oturumda otomatik yüklenir. Başka hiçbir doküman otomatik yüklenmez — §0.1 indeksindeki "önce oku"ları **sen açarsın**.
- `.Codex/memory/` Codex tarafından instruction olarak **otomatik yüklenmez**; canlı memory store'un git-aynasıdır (senkron: `.Codex/README.md`). Kalıcı kural buraya değil, AGENTS.md/INVARIANTS'a yazılır.

### 18.2 Slash komutları (`.Codex/commands/`)
...
### 18.4 Güvenlik izinleri (`.Codex/settings.json`)
