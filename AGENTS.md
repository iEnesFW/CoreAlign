...
>
> **Bu dosya nasıl okunur (her task başında):** (1) bu çekirdeği oku; (2) `docs/INVARIANTS.md` dosyasını açıp bir-daha-aynı-hatayı-yapma kayıtlarını uygula; (3) dokunacağın alanı **"Sistem Haritası & Modül Guardrail İndeksi"**nde (§0.1) bul, oradaki tuzakları ve "önce-oku" dokümanını aç. Bir işi yaparken başka bir modülü bozmamanın yolu bu indekstir.
...
> **Amaç:** "Bir işi yaparken diğerini bozma." Dokunacağın alanı bul; **Tuzaklar**'ı uygula, **Önce oku**'yu aç. Detay kural numaraları (§x.y) bu dosyadadır; `INV` = task başında okunması zorunlu `docs/INVARIANTS.md`.
...
| **Cam Mekan (Glass Enclosure)**                                | `Application/GlassEnclosure` (141 dosya), `src/features/glass-enclosure` (157), `src/shared/three-engine` | **Büyük modül**; 3D tasarımcı (three.js r128 sınırı); `IHasConcurrencyToken`; `glass_*_change_logs` partition adayı; `sky` domain rengi korunur                                                          | `docs/Cam_Mekan_Modul_Plan.md`; `.claude/memory/project_corealign_design_system.md`                    |
...
| **Hata yönetimi · Gözlemlenebilirlik (çapraz)**                | `ExceptionHandlingMiddleware`, `error_logs`, `src/shared/errors`                                          | `ApiResponse<T>` zarfı; exception fırlat, status kodlama; 5xx detayı sızdırma; correlation tek id; `Add*/Use*` çağrılı                                                                                   | §3.4, §2.4; `docs/modules/error-handling.md`; `.claude/memory/project_corealign_error_handling.md`     |
| **Frontend (4 yüzey · design system · i18n)**                  | §0 tablosu                                                                                                | Doğru yüzey; FSD yön kuralı; `t()` + tr&en senkron; dark+responsive; `primary-*` token (raw `indigo-*` değil)                                                                                            | §2; `docs/DESIGN_SYSTEM.md`; `.claude/memory/project_corealign_design_system.md`                       |
...
6. **Çatışma çözümü.** Bir invariant AGENTS.md ile çelişirse AGENTS.md kazanır; invariant güncellenir veya silinir.
...
## 18. Codex Çalışma Düzeni (bu repo)

CoreAlign'a özel Codex varlıkları `.agents/` ve `.codex/` altında repoyla taşınır.

### 18.1 Yüklenen bağlam

- `AGENTS.md` otomatik yüklenir. Codex `@import` sözdizimini uygulamadığı için `docs/INVARIANTS.md` dosyasını her task başında açıkça oku.
- `.claude/memory/` geçmiş karar kayıtlarıdır ve otomatik yüklenmez. Tasarım sistemi, hata yönetimi veya bildirim entegrasyonu görevinde `$corealign-project-context` skill'ini kullan. Kalıcı kuralı memory kaydına değil `AGENTS.md` veya `docs/INVARIANTS.md` dosyasına yaz.

### 18.2 Skill'ler (`.agents/skills/`)

- **`$source-command-pre-ship`** — bitirmeden önce build, lint, typecheck, ilgili testler ve gerekiyorsa tabula-rasa migration doğrulaması.
- **`$source-command-new-endpoint`** — slim controller, Application, validator, tenant filter, cross-tenant ve N+1 testleri, i18n.
- **`$source-command-new-module`** — doğru frontend yüzeyiyle FSD ve CQRS modül akışı.
- **`$source-command-db-migration`** — ileri-tarihli Phase ID, idempotency, aynı-tur uygulama, snapshot drift ve tabula-rasa protokolü.

### 18.3 Reviewer subagent (`.codex/agents/`)

- **`corealign-reviewer`** — değişiklik sonrası §8 checklist'ini bağımsız uygular. Bitti demeden önce inceleme istendiğinde çalıştır; kod yazmaz, kanıtlı bulgular raporlar.

### 18.4 Güvenlik kuralları (`.codex/rules/default.rules`)

- Build, test, lint, typecheck ve salt-okunur Git komutları izinlidir. Yıkıcı Git, DB ve container komutları açık onay ister. Execpolicy kuralları sandbox dışında komut çalıştırmayı yönetir; §7 davranış kuralları ayrıca bağlayıcıdır.
