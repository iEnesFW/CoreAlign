...
| **§214/§220 (YÜKSEK)** MRP dashboard/suggestion sınırsız `ToListAsync`/`int.MaxValue` | Legacy MrpService sorgu-yeniden-yazımı (aday filtresini SQL'e taşı); anlamlı refactor, dashboard hot-path değil ama gerçek.                                                                                                                                                       |
