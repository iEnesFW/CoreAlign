...
**O4. Tabula-rasa migration testi (§1.1.2) — GEÇTİ + idempotent-script uyarısı** `[verified + note]`

- Tüm 112 migration sıfırdan temiz `corealign_tabularasa` DB'sine **canonical path ile hatasız** uygulandı (ON_ERROR_STOP). Sonuç şema doğru: 5 partition parent (activity_logs/login_audit_logs/outbox_messages/stock_movements/customer_transactions) + notification_messages un-partitioned (Phase87), Phase89 fonksiyonu UTC-pinned, 4 Phase89 index, RLS 150 tabloda, 112/112 history.
- **Uyarı:** `dotnet ef migrations script --idempotent` çıktısı her migration'ı `DO $EF$ ... END $EF$` (plpgsql) içine sarar; Phase86 (`SELECT corealign_partition_leaf_table(...)`) ve Phase87 (`SELECT corealign_unpartition_table(...)`) partition helper'larını bare `SELECT` ile çağırıyor — plpgsql içinde bare `SELECT` "query has no destination" hatası verir (PERFORM gerekir, ama PERFORM top-level'da geçersiz; ikisinde de çalışan tek form yok). Yani **deploy `ef database update` / `dotnet ef migrations bundle` / non-idempotent script ile yapılmalı, `--idempotent` ile DEĞİL.** Canonical path'ler çalışıyor; idempotent-script desteği Phase86/87'nin statik-DDL'e yeniden yazılmasını gerektirir (applied migration, riskli) → ihtiyaç olursa ayrı görev.
