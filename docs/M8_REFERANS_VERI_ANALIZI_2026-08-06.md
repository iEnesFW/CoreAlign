...
| -------------------------------------------------------------------------- | ------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `Application/EInvoice/UblTrInvoiceXmlBuilder.cs:335,363`                   | `unitCode="{UomCode ?? "C62"}"` | ham metni geçirir | ❌ **gerçek fatura yolu** — `Kg`, `ADET` gibi geçersiz kod → entegratör reddi                                                   |
| `Application/EInvoice/UblTrInvoiceXmlBuilder.cs:122` (irsaliye)            | `unitCode="C62"` sabit          | her şey "adet"    | ⚠️ `ShipmentLine` birim taşımıyor; düzeltmek sipariş satırına inmeyi gerektirir (takip)                                         |
| `Infrastructure/.../Common/UblTrInvoiceBuilder.cs:186,200` + `Foriba…:323` | `unitCode="C62"` sabit          | her şey "adet"    | ℹ️ **stub/sandbox yolu** — gerçek gönderimde `EFaturaDocument.Lines` daima boş (`Array.Empty`), belge `RawUblTrXml` ile taşınır |
...
Faz 2'nin beş maddesinden dördü aynı gün teslim edildi; kalan madde (para birimini `currencies` kataloğuna karşı doğrulama) §7-1 kararına bağlı olduğu için bekletildi.

| #   | Madde                              | Durum | Not                                                                                                                                                                                                                       |
| --- | ---------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5   | `GibUnitCodeMap` + tek UBL kapısı  | ✅    | `Application/EInvoice/GibUnitCodeMap.cs`; üç yazma noktası da buradan geçiyor. Eşleşmeyen birim artık `UnmappedUnitCodeException` (→400) ile belgeyi durduruyor, sessizce C62'ye düşmüyor.                                |
...
| 3   | Üretim işi birimi açılır listesi   | ✅    | Aynı bileşen; varsayılan `'PCS'` → `'ADET'` (küratörlü kod).                                                                                                                                                              |
| 1   | İçe aktarmada birim doğrulaması    | ✅    | `CreateProductCommandValidator`/`UpdateProductCommandValidator`'a `Validation.UnitNotRecognized` kuralı eklendi; `ProductBulkImporter` bu validator'ı zaten çağırdığı için hem API hem CSV/XLSX yolu tek kuralla kapandı. |
| 4   | Para birimi katalog doğrulaması    | ⏸     | Ürün validator'ında ISO-4217 **şekil** kuralı (`^[A-Z]{3}# M8 · Referans Veri Disiplini — Faz 1 Analiz Raporu                                                                                                             |
...
) zaten vardı. Katalog kontrolü §7-1 kararını bekliyor (aksi halde kuru olan 17 para birimi reddedilir). |

Yan koruma (Faz 4'ten öne alındı, çünkü Faz 2 birim alfabesini değiştirebilir hale getirdi): `GlassLineMath.AreaUnitDivisor` ve FE ikizi `orderSchema.areaUnitDivisor` artık küratörlü kodları (`METREKARE`, `SANTIMETREKARE`, `DESIMETREKARE`, `MILIMETREKARE`) ve GİB kodlarını (`MTK`, `CMK`, `DMK`, `MMK`) da tanıyor — birim listeden seçilmeye başlandığında cam m² türetmesi sessizce durmasın diye.

Doğrulama: dotnet build 0 uyarı · Application 2572/2572 · Integration 271 pass (5 pre-existing skip) · tsc 0 · eslint 0 · vitest 964/964 · i18n parite 6854=6854 · vite build + prerender.
