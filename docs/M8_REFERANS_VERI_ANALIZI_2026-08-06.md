...
**Cam alan hesabı tuzağı (kod okunarak doğrulandı — `GlassLineMath.cs:11-22`):** alan birimi string eşleşmesiyle tanınır; kabul edilen liste `m2 / sqm / metrekare / dm2 / cm2 / mm2` (trim + küçük harf + kare-işareti sadeleştirmesi sonrası). Bugünkü `M2` tanınır, küratörlü `METREKARE` tesadüfen tanınır, ama `SANTIMETREKARE` ve GİB kodu `MTK` **TANINMAZ**. Birim alfabesi değiştirilirken bu fonksiyon aynı commit içinde güncellenmezse cam satırlarında alan türetmesi sessizce durur ve girilen miktar adet gibi işlenir.
...
| Yol                                                                        | Kod                        | Davranış          | Gerçek etki                                                                                                                     |
| -------------------------------------------------------------------------- | -------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `Application/EInvoice/UblTrInvoiceXmlBuilder.cs:335,363`                   | ham `UomCode`, yoksa `C62` | ham metni geçirir | ❌ **gerçek fatura yolu** — `Kg`, `ADET` gibi geçersiz kod → entegratör reddi                                                   |
| `Application/EInvoice/UblTrInvoiceXmlBuilder.cs:122` (irsaliye)            | `C62` sabit                | her şey "adet"    | ⚠️ `ShipmentLine` birim taşımıyor; düzeltmek sipariş satırına inmeyi gerektirir (takip)                                         |
| `Infrastructure/.../Common/UblTrInvoiceBuilder.cs:186,200` + `Foriba…:323` | `C62` sabit                | her şey "adet"    | ℹ️ **stub/sandbox yolu** — gerçek gönderimde `EFaturaDocument.Lines` daima boş (`Array.Empty`), belge `RawUblTrXml` ile taşınır |

Yani "iki kurucu farklı davranıyor" sorunu pratikte tek yolu etkiliyor: gerçek fatura **her zaman** `Application` kurucusundan çıkar; Infrastructure kurucuları yalnız sağlayıcı test/sandbox akışında devrede.
...
## 6.1 Faz 2 — UYGULANDI (2026-08-06)

Faz 2'nin beş maddesinden dördü aynı gün teslim edildi.

| #   | Madde                              | Durum     | Not                                                                                                                                                                                      |
| --- | ---------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5   | `GibUnitCodeMap` + tek UBL kapısı  | ✅        | `Application/EInvoice/GibUnitCodeMap.cs`; üç yazma noktası da buradan geçiyor. Eşleşmeyen birim `UnmappedUnitCodeException` (→400) ile belgeyi durduruyor, sessizce C62'ye düşmüyor.     |
| 2   | Ürün formunda birim açılır listesi | ✅        | `shared/ui/form/UnitOfMeasureSelect` (CurrencySelect deseni: saklı bilinmeyen değeri kendi seçeneği olarak korur).                                                                       |
| 3   | Üretim işi birimi açılır listesi   | ✅        | Aynı bileşen; varsayılan küratörlü koda çekildi.                                                                                                                                         |
| 1   | İçe aktarmada birim doğrulaması    | ✅        | `Create/UpdateProductCommandValidator`'a `Validation.UnitNotRecognized` kuralı; `ProductBulkImporter` bu validator'ı zaten çağırdığı için hem API hem CSV/XLSX yolu tek kuralla kapandı. |
| 4   | Para birimi katalog doğrulaması    | ⏸ → Faz 3 | Ürün validator'ında ISO-4217 şekil kuralı zaten vardı; katalog kontrolü §7-1 kararına bağlıydı, karar geldi (aşağı).                                                                     |

Yan koruma (Faz 4'ten öne alındı): `GlassLineMath.AreaUnitDivisor` ve FE ikizi `orderSchema.areaUnitDivisor` artık küratörlü kodları (`METREKARE`, `SANTIMETREKARE`, `DESIMETREKARE`, `MILIMETREKARE`) ve GİB kodlarını (`MTK`, `CMK`, `DMK`, `MMK`) da tanıyor.

Doğrulama: dotnet build 0 uyarı · Application 2572/2572 · Integration 271 pass · tsc 0 · eslint 0 · vitest 964/964 · i18n parite 6854=6854 · vite build + prerender · canlı sonda 3/3.

---

## 7. Kararlar (kullanıcı, 2026-08-06)

| #   | Soru                                                                   | KARAR                                                            | Uygulama sonucu                                                                                                                                                                                                                                                    |
| --- | ---------------------------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Para birimi kataloğu elle mi genişlesin, kur beslemesinden mi türesin? | **Varsayılan kur beslemesinden TÜRESİN, elle genişletilebilsin** | TCMB ingest'i getirdiği kodları `currencies`'e upsert eder (TRY taban olarak her zaman kalır); elle ekleme korunur; katalog dolduğu için Faz-2'de ertelenen "para birimini kataloğa karşı doğrula" kuralı güvenle açılabilir.                                      |
| 2   | Birim alfabesi uzun ad mı kısaltma mı?                                 | **Kısaltmalar (GİB / UN-ECE Rec-20)**                            | Küratörlü kodlar `KGM`, `MTK`, `C62`, `MTR`, `LTR`… olur; `products.unit` backfill edilir; kullanıcıya kod değil **sembol — ad** gösterilir (tablo zaten `name` + `symbol` taşıyor). `GibUnitCodeMap` eski serbest metinler için geriye-uyum katmanı olarak KALIR. |
| 3   | "Sıfırlama" hangi anlamda?                                             | **Belirsiz — atlanıyor**                                         | Kullanıcının net beklentisi yok; yıl-sıfırlama/devir semantiği bu turda ELLENMEZ. Mevcut yıl sonu kapanış/açılış özelliği olduğu gibi kalır.                                                                                                                       |
| 4   | Mali yıl kullanıcı bazlı mı tenant bazlı mı?                           | **Firma (tenant) bazlı**                                         | Aktif çalışma yılı tenant ayarıdır; tüm kullanıcılar aynı yılı görür.                                                                                                                                                                                              |

### 7.1 Kararların açtığı yeni ölçümler (canlı DB, 2026-08-06)

- Kur beslemesinde olup katalogda **olmayan 18 kod**: AED AUD AZN CAD CHF CNY DKK JPY KRW KWD KZT NOK PKR QAR RON RUB SAR SEK. Katalogda olup beslemede olmayan tek kod `TRY` (taban para birimi — doğru).
- `products.base_uom_id / sales_uom_id / purchase_uom_id` **60/60 üründe NULL** → küratörlü FK modeli fiilen hiç kullanılmamış; yalnız serbest metin `unit` taşınıyor. Backfill yüzeyi bu yüzden küçük.
- Kalem tablolarında `uom_code` dolu satır sayısı: `order_lines` **1**, diğerlerinin hepsi **0**.
- ⚠️ **Dördüncü bir alfabe bulundu:** cam modülü kendi birim metinlerini taşıyor — `glass_hardware_items`: `Meter` (2), `Piece` (10); `glass_project_bom_lines`: `adet` (5), `lot` (12), `m` (71), `m²` (43). `lot` bir ölçü birimi DEĞİL (parti). Bunlar bugün `GibUnitCodeMap`'e uğramıyor ama BOM bir belgeye beslenirse hata atar → haritaya İngilizce takma adlar ve `lot` için bilinçli bir karar eklenmeli.
