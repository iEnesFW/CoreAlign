...
- **Faz 4 (kısmen TAMAM):** Public yüzey (landing + auth) premium yeniden tasarımı. **Konvansiyonlar:** (1) **Display font** = Sora (`--font-display`, `@fontsource-variable/sora`); body Inter kalır. Sora yalnız public yüzeyde — kök elemana `ca-marketing` class'ı eklenir, `.ca-marketing :is(h1,h2,h3,h4,.ca-display)` Sora uygular (admin UI Inter kalır). (2) Landing **tam genişlik**; eski sürekli login-sidebar kaldırıldı, giriş üst nav'daki "Giriş" → `/login` (AuthLayout) ile. (3) Yüzey sistemi: `.ca-page-bg` (sayfa zemini), `.ca-panel` (cam kart), `.ca-card-hover` (hover lift), `.ca-blueprint` (mühendislik grid), `.ca-aurora` (renk alanı), `.ca-text-gradient`. (4) Animasyonlar `animate-{float,float-sm,scan-y,spin-slow}` + `ca-marquee-track` + `ca-draw-line`; hepsi `prefers-reduced-motion` ile kapanır. (5) Hero görseli (`HeroShowcase`) SVG+CSS — three.js landing'e MOUNT EDİLMEZ (bundle/SEO). (6) SEO sözleşmesi korunur: `prerender-seo.mjs` `LandingPage.hero.title`/`solutions.title`/`about.title`/`articles.title`/`contact.title` (h1) + `showcase.m{1..6}Title` + `faq.q#/a#` okur — bu anahtarlar silinmez. **Kalan:** homepage alt-section'larının (Migration/Integrations/Security/Savings/Testimonials/Faq/Demo) tek tek cila turu; gerçek müşteri sosyal kanıtı.
