...
- [TEST] N+1 regresyonunu CI'da yakalamak için EF Core `DbCommandInterceptor` tabanlı bir round-trip-sayıcı henüz devrede değil → entegrasyon testlerine `IDbCommandInterceptor` ekleyip beklenen-üst-sınırı assert eden bir `[Fact]` zinciri planlanmalı (takip: docs/sprint10-blockers.md).
