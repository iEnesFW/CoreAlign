...
export const Topbar = ({ onOpenSidebar }: TopbarProps) => {
  ...
  const availableLanguages = useMemo(() => visibleLanguages(loadGatedLocales()), []);
    ...
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between gap-3 border-b border-slate-100 bg-white/80 px-4 backdrop-blur lg:px-8 dark:border-slate-800 dark:bg-slate-950/80">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-sky-500 to-indigo-600 text-white shadow-md shadow-sky-500/20">
            <Sparkles size={16} />
          </span>
          <div className="hidden flex-col sm:flex">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">{t('app.tagline')}</span>
          </div>
        </div>
      </div>
