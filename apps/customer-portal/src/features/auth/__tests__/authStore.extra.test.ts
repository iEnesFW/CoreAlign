...
describe('useAuthStore extra coverage', () => {
  ...
  it('restore rehydrates state when token is live', () => {
    ...
    window.localStorage.setItem('corealign.portal.token', 'tok');
    window.localStorage.setItem('corealign.portal.tokenExpires', expires);
  ...
  it('restore is resilient to corrupt JSON in user blob', () => {
    ...
    window.localStorage.setItem('corealign.portal.token', 'tok');
    window.localStorage.setItem('corealign.portal.tokenExpires', expires);
    window.localStorage.setItem('corealign.portal.user', 'not-json');
    useAuthStore.getState().restore();
    expect(useAuthStore.getState().isAuthenticated).toBe(false);
  });
  ...
  it('setAuth persists JSON-serialized user', () => {
    ...
    const stored = window.localStorage.getItem('corealign.portal.user');
