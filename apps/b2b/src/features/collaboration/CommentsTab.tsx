...
export const CommentsTab = ({ orderId }: CommentsTabProps) => {
  ...
  const submit = () => {
    ...
    if (!trimmed) {
      toast.error(t('b2b.comments.empty'));
      return;
    }
    postMutation.mutate(trimmed, {
      onSuccess: () => {
        setDraft('');
        toast.success(t('b2b.comments.posted'));
      },
    ...
    <div className="flex flex-col gap-3">
      <header className="flex items-center justify-between">
        <h3 className="inline-flex items-center gap-2 text-sm font-semibold text-slate-800 dark:text-slate-100">
          ...
          {t('b2b.comments.title')}
        </h3>
        <span className="text-xs text-slate-500 dark:text-slate-400">
          {t('b2b.comments.count', { count: items.length })}
        </span>
      </header>
              ...
              <li key={comment.id} className="px-4 py-3">
                <div className="flex items-baseline justify-between gap-3">
                  <span className="text-xs font-semibold text-slate-800 dark:text-slate-100">
                    {comment.authorName || t('b2b.comments.unknownAuthor')}
                  </span>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
        <textarea
          ...
          placeholder={t('b2b.comments.placeholder')}
          rows={2}
          maxLength={4000}
          disabled={posting}
