...
interface Props {
  ...
  /** Not used for country-state-city since it is globally aware, kept for backwards compatibility */
 ...
 * country-state-city package. Values are stored as human-readable names.
 */
export const AddressRegionFields = ({
  ...
  useEffect(() => {
    if (!country && detectedCountryName && !hasAutoFilled.current) {
      ...
      // timeout ensures it runs after parent form reset cycles on mount
      setTimeout(() => onCountryChange(detectedCountryName), 10);
    }
