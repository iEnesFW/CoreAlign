...
import { CheckCircle2, ChevronRight, Play, X, SkipForward, RotateCcw, Box } from 'lucide-react';
      ...
      <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-800">
        <div>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
            {job.jobNumber}
            <Badge tone="neutral">{t(`ProductionJobs.status.${job.status}`)}</Badge>
          </h2>
          <p className="text-sm text-slate-500">{job.productName} — {job.plannedQuantity} {job.unitOfMeasure}</p>
        </div>
        ...
        {job.status === 'Draft' && (
          <div className="bg-white p-4 rounded-xl border border-slate-200 dark:bg-slate-900 dark:border-slate-800 flex gap-4 items-end shadow-sm">
            <div className="flex-1">
              <Select label={t('ProductionJobs.fields.warehouse')} value={warehouseId} onChange={e => setWarehouseId(e.target.value)}>
...
const StepCard = ({ job, step }: { job: any; step: any }) => {
  const { t } = useTranslation();
  ...
  const handleStart = async () => {
    if (!operatorId) { toast.error(t('ProductionJobs.operator_required')); return; }
    try {
  ...
  const isActive = job.currentStepNumber === step.stepNumber && (job.status === 'Released' || job.status === 'InProgress');
  ...
  return (
    <div className={`relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active`}>
        {/* Timeline dot */}
        <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-white dark:border-slate-900 bg-white dark:bg-slate-900 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 ${isActive ? 'ring-2 ring-primary-500' : ''}`}>
           {isCompleted ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : 
            isSkipped ? <SkipForward className="w-5 h-5 text-slate-400" /> :
            <div className={`w-3 h-3 rounded-full ${isActive ? 'bg-primary-500 animate-pulse' : 'bg-slate-300 dark:bg-slate-600'}`} />}
        </div>
        ...
        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="flex justify-between items-start mb-2">
               <div>
                  <div className="text-xs font-semibold text-primary-600 dark:text-primary-400 tracking-wide uppercase">{t('ProductionJobs.step')} {step.stepNumber}</div>
                  <h4 className="font-medium text-slate-900 dark:text-white mt-1">{step.operationName}</h4>
                  <p className="text-sm text-slate-500">{step.workCenterName}</p>
               </div>
               <Badge tone={isCompleted ? 'success' : isSkipped ? 'neutral' : isActive ? 'warning' : 'neutral'}>
                 {t(`ProductionJobs.stepStatus.${step.status}`)}
               </Badge>
            </div>
            
            {step.instructions && <div className="text-sm bg-slate-50 dark:bg-slate-800 p-2 rounded mb-3 text-slate-600 dark:text-slate-300">{step.instructions}</div>}
            ...
            {isActive && step.status === 'Pending' && (
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
                 <Select value={operatorId} onChange={e => setOperatorId(e.target.value)} size="sm">
                    ...
                    {operators?.map(o => <option key={o.id} value={o.employeeId}>{o.employeeName}</option>)}
                 </Select>
                 <div className="flex gap-2">
                   <Button size="sm" onClick={handleStart} className="flex-1"><Play className="w-4 h-4" /> {t('ProductionJobs.actions.start')}</Button>
                   {step.isOptional && <Button size="sm" variant="ghost" onClick={handleSkip}><SkipForward className="w-4 h-4" /></Button>}
                 </div>
              </div>
            )}
            ...
            {isActive && step.status === 'InProgress' && (
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
                 ...
                 <div className="grid grid-cols-2 gap-2">
                    <Input type="number" label={t('ProductionJobs.fields.good')} value={goodQty} onChange={e => setGoodQty(Number(e.target.value))} size="sm" />
                    <Input type="number" label={t('ProductionJobs.fields.scrapped')} value={scrappedQty} onChange={e => setScrappedQty(Number(e.target.value))} size="sm" />
                 </div>
                 <Button size="sm" onClick={handleFinish} tone="success" className="w-full"><CheckCircle2 className="w-4 h-4" /> {t('ProductionJobs.actions.finish')}</Button>
              </div>
            )}

            {isCompleted && (
              <div className="mt-3 text-sm text-slate-500 grid grid-cols-2 gap-2">
                 <div>{t('ProductionJobs.fields.good')}: <span className="font-medium text-emerald-600">{step.goodQuantity}</span></div>
                 {step.scrappedQuantity > 0 && <div>{t('ProductionJobs.fields.scrapped')}: <span className="font-medium text-red-600">{step.scrappedQuantity}</span></div>}
              </div>
            )}
        </div>
    </div>
