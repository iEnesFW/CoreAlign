...
export const IssueCreditNoteModal = ({ invoice, open, onClose, onSuccess }: Props) => {
  ...
  const creditedQuery = useCreditedQuantitiesByLine(open && invoice ? invoice.id : null);

  const alreadyCreditedByLine = useMemo(() => {
    const map = new Map<string, number>();
    for (const row of creditedQuery.data?.data ?? []) {
      map.set(row.invoiceLineId, row.creditedQuantity);
    }
    return map;
  }, [creditedQuery.data]);
  ...
  const creditedSettled = creditedQuery.isSuccess || creditedQuery.isError;
  const resetKey = activeKey !== null && creditedSettled ? activeKey : null;
  ...
  const [trackedKey, setTrackedKey] = useState<string | null>(resetKey);
  ...
  if (trackedKey !== resetKey) {
    ...
    setSelections(resetKey !== null ? initialSelections : {});
    setReason('');
  }
  ...
  const submit = async () => {
    ...
    try {
      const response = await issueMutation.mutateAsync({
        id: invoice.id,
        payload: { lines: chosen, reason: reason.trim() || null, operationId: newOperationId() },
      });
  ...
  return (
    <Modal
      ...
      icon={<FileMinus size={16} className="text-danger-500" />}
      size="2xl"
      footer={
      ...
      <div className="space-y-4 p-4">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {lines.map((line) => {
                  ...
                  return (
                    <tr
                      key={line.id}
                      className={sel.selected ? 'bg-danger-50/40 dark:bg-danger-500/5' : ''}
                    >
                      ...
                      <td className="px-2 py-2 text-right">
                        <input
                          ...
                          className="w-24 rounded border border-slate-300 px-2 py-1 text-right text-xs tabular-nums focus:border-primary-500 focus:outline-none disabled:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:disabled:bg-slate-900"
                        />
                      </td>
                      <td className="px-2 py-2 text-right tabular-nums text-slate-700 dark:text-slate-300">
        ...
        <div>
          ...
          <textarea
            ...
            className="block w-full rounded border border-slate-300 px-2 py-1.5 text-xs focus:border-primary-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800"
          />
        </div>
      </div>
    </Modal>
