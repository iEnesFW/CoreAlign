...
import { ArrowDown, ArrowUp, Filter, Hash } from 'lucide-react';
...
export const StockMovementsTab = ({ productId }: Props) => {
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="inline-flex items-center gap-1 text-xs text-slate-600 dark:text-slate-400">
          <Filter size={12} />
          {t('inventory.movements.filterByType')}
        </div>
        <select
          value={type}
          onChange={(e) => {
        ...
        <span className="ml-auto text-[11px] text-slate-500 dark:text-slate-400">
          {t('inventory.movements.totalCount', { count: total })}
        </span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50/60 text-[10px] uppercase tracking-wider text-slate-500 dark:bg-slate-900/30 dark:text-slate-400">
              <tr>
                ...
                <th className="px-3 py-2 text-left">{t('inventory.movements.type')}</th>
                <th className="px-3 py-2 text-left">{t('inventory.movements.warehouse')}</th>
                <th className="px-3 py-2 text-right">{t('inventory.movements.quantity')}</th>
                <th className="px-3 py-2 text-right">{t('inventory.movements.onHandAfter')}</th>
                <th className="px-3 py-2 text-right">{t('inventory.movements.unitCost')}</th>
                <th className="px-3 py-2 text-left">{t('inventory.movements.source')}</th>
                <th className="px-3 py-2 text-left">{t('inventory.movements.reason')}</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={m.id} className="hover:bg-slate-50/40 dark:hover:bg-slate-800/30">
                    ...
                    <td className="px-3 py-2 text-slate-700 dark:text-slate-300">
                      ...
                      {m.lotNumber && (
                        <div className="mt-0.5 text-[10px] text-violet-600 dark:text-violet-400">
                          {t('inventory.fields.lot')}: {m.lotNumber}
                        </div>
                      )}
                    </td>
                    <td
                    ...
                    <td className="px-3 py-2 text-[11px] text-slate-600 dark:text-slate-400">
                      {m.reasonCodeName ?? '—'}
                      {m.notes && (
                        <div className="text-[10px] italic text-slate-500">{m.notes}</div>
                      )}
                    </td>
                  </tr>
                );
