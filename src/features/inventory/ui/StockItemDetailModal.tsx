...
const VelocityCard = ({
  ...
  const abcTone: Record<'A' | 'B' | 'C' | 'D', { bg: string; text: string }> = {
    ...
    B: { bg: 'bg-blue-100 dark:bg-blue-500/20', text: 'text-blue-700 dark:text-blue-300' },
    C: { bg: 'bg-amber-100 dark:bg-amber-500/20', text: 'text-amber-700 dark:text-amber-300' },
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4 text-[11px]">
        ...
        <div className="rounded border border-slate-200 px-2 py-1.5 dark:border-slate-800">
          ...
          <div
            className={`mt-0.5 font-bold tabular-nums ${stats.netDelta > 0 ? 'text-emerald-600 dark:text-emerald-400' : stats.netDelta < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-200'}`}
          >
            {stats.netDelta > 0 ? '+' : ''}
