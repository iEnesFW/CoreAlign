...
const variantConfig = {
    success: {
        icon: CheckCircle2,
        containerClass: 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/20',
...
export const Alert: React.FC<AlertProps> = ({ variant, title, message, className }) => {
    ...
    const displayTitle = title || t(config.defaultTitleKey as 'common.success' | 'common.info' | 'common.warning' | 'common.error');

    return (
        <div className={cn("flex items-start gap-2.5 p-3 rounded-[5px] border", config.containerClass, className)}>
