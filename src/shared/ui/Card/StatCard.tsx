...
export const StatCard: React.FC<StatCardProps> = ({ name, value, change, trend, icon: Icon, color }) => {
    return (
        <div className="relative overflow-hidden p-3 bg-white/50 dark:bg-[#0B0F19]/50 backdrop-blur-md rounded-[5px] border border-slate-200/60 dark:border-slate-800/60 transition-all duration-300 hover:border-indigo-500/50 group flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 relative z-10">
                <div className={`p-2 rounded-[5px] bg-gradient-to-br ${color} text-white shadow-lg shadow-${color.split('-')[1]}-500/30 group-hover:scale-110 transition-transform duration-300`}>
            ...
            <div className="relative z-10 flex flex-col items-end">
                <div
                    className={`flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded-[3px] backdrop-blur-sm ${trend === 'up'
                ...
                <div className="mt-1.5 flex items-end gap-0.5 h-3 opacity-50 group-hover:opacity-100 transition-opacity">
                    {[40, 70, 45, 90, 65, 85, 100].map((h, i) => (
                        <div key={i} className={`w-1 rounded-t-sm bg-gradient-to-t ${color}`} style={{ height: `${h}%` }} />
                    ))}
                </div>
            </div>
        </div>
