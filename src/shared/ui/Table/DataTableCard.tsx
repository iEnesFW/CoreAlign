        ...
        <div className="bg-white dark:bg-[#0B0F19] rounded-[5px] shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] dark:shadow-none border border-slate-200/60 dark:border-slate-800/60 flex flex-col">
                    ...
                    <thead>
                        <tr className="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-200/60 dark:border-slate-800/60">
                            <th className="p-2 w-8 text-center">
                                <input type="checkbox" className="rounded-[3px] border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                            </th>
                            <th className="p-2 text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Order ID</th>
                            <th className="p-2 text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Customer</th>
                            <th className="p-2 text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Amount</th>
                            <th className="p-2 text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                            <th className="p-2 text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Date</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200/60 dark:divide-slate-800/60">
                        {data.map((row) => (
                            <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                                ...
                                <td className="p-2 text-[11px] font-medium text-slate-900 dark:text-white">{row.id}</td>
                                <td className="p-2 text-[11px] text-slate-600 dark:text-slate-300">{row.customer}</td>
                                <td className="p-2 text-[11px] font-medium text-slate-900 dark:text-white">{row.amount}</td>
                                <td className="p-2">
                                    <span className={`inline-flex items-center px-1.5 py-0.5 rounded-[3px] text-[9px] font-bold uppercase tracking-wider ${row.status === 'Completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' :
                                        row.status === 'Pending' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' :
                                            row.status === 'Processing' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400' :
            ...
            <div className="p-2 border-t border-slate-200/60 dark:border-slate-800/60 flex items-center justify-between bg-slate-50/30 dark:bg-slate-800/10">
                <span className="text-[10px] text-slate-500 dark:text-slate-400">
                    {t('table.showing')} <span className="font-medium text-slate-900 dark:text-white">1</span> {t('table.to')} <span className="font-medium text-slate-900 dark:text-white">5</span> {t('table.of')} <span className="font-medium text-slate-900 dark:text-white">24</span> {t('table.results')}
                </span>
                <div className="flex items-center gap-1">
                    ...
                    <button className="w-5 h-5 flex items-center justify-center rounded-[3px] text-[10px] font-medium bg-indigo-600 text-white">1</button>
                    <button className="w-5 h-5 flex items-center justify-center rounded-[3px] text-[10px] font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">2</button>
                    <button className="w-5 h-5 flex items-center justify-center rounded-[3px] text-[10px] font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">3</button>
