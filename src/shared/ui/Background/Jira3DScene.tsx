...
export const Jira3DScene = ({ theme = 'dark' }: Jira3DSceneProps) => {
    ...
    useEffect(() => {
        ...
        folderGeneral.add(params, 'globalRotation', -180, 180).name('Rotation').onChange(updateGroupPositions);
        folderGeneral.add(params, 'positionX', -200, 200).name('Position X').onChange(updateGroupPositions);
        folderGeneral.add(params, 'positionY', -100, 100).name('Position Y').onChange(updateGroupPositions);
