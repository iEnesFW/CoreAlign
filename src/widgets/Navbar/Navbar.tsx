...
export const Navbar: React.FC<NavbarProps> = ({ toggleSidebar }) => {
    ...
    const initials = user && user.firstName && user.lastName ? `${user.firstName[0]}${user.lastName[0]}`.toUpperCase() : 'U';
                ...
                <div className="relative ml-1" ref={profileRef}>
                    ...
                    >
                        ...
                        <div className="hidden sm:block text-left">
                            <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 leading-none">{name}</p>
                            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 font-medium">Administrator</p>
                        </div>
                    </button>
                    ...
                    {isProfileOpen && (
                        <div className="absolute right-0 mt-2 w-48 rounded-[5px] shadow-lg bg-white dark:bg-slate-800 ring-1 ring-slate-200 dark:ring-slate-700 divide-y divide-slate-100 dark:divide-slate-700/50 focus:outline-none py-1 transform opacity-100 scale-100 transition-all duration-200 origin-top-right">
                            ...
                            <div className="px-1 py-1">
                                <a href="#profile" className="group flex items-center px-2 py-1.5 text-[11px] font-medium text-slate-700 dark:text-slate-300 rounded-[5px] hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                    <User className="mr-2 h-3.5 w-3.5 text-slate-400 group-hover:text-indigo-500 transition-colors" />
                                    My Profile
                                </a>
                                <a href="#settings" className="group flex items-center px-2 py-1.5 text-[11px] font-medium text-slate-700 dark:text-slate-300 rounded-[5px] hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                    <Settings className="mr-2 h-3.5 w-3.5 text-slate-400 group-hover:text-indigo-500 transition-colors" />
                                    Account Settings
                                </a>
                                <a href="#parameters" className="group flex items-center px-2 py-1.5 text-[11px] font-medium text-slate-700 dark:text-slate-300 rounded-[5px] hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
