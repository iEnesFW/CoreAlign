...
const navigation: NavItem[] = [
    ...
    { section: 'SALES & CRM' },
    {
        ...
        children: [
            ...
            { name: 'New Customer', href: '/dashboard/customers/new' }
        ]
    },
    {
        name: 'Orders',
        ...
        children: [
            ...
            { name: 'Pending Orders', href: '/dashboard/orders/pending' }
        ]
    },
    {
        name: 'Receipts',
    ...
    { section: 'FINANCE' },
    {
        ...
        icon: FileText,
        children: [
            { name: 'Invoice List', href: '/dashboard/invoices' },
        ...
        <>
            ...
            >
                ...
                <div className="flex items-center justify-between h-12 px-3 border-b border-slate-200/60 dark:border-slate-800/60 shrink-0">
                    <div className={`flex items-center gap-2 font-bold text-sm text-slate-900 dark:text-white transition-opacity duration-300 ${isCollapsed ? 'lg:opacity-0 lg:w-0 lg:overflow-hidden' : 'opacity-100'}`}>
                        ...
                        <span className="tracking-tight">Nexus<span className="text-indigo-500">ERP</span></span>
                    </div>

                    {/* Logo for collapsed state */}
                    <div className={`hidden lg:flex items-center justify-center absolute left-0 w-full transition-opacity duration-300 ${isCollapsed ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    ...
                    {navigation.map((item, index) => {
                            ...
                            return (
                                <div key={item.name} className="space-y-[2px]">
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between px-2 py-1.5 rounded-[5px] transition-all duration-200 group ${isExpanded
                                            ? 'bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white'
                                            : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white'
                                            }`}
                                    >
                                        <div className="flex items-center gap-2">
                                            <Icon size={16} className={`transition-colors ${isExpanded ? 'text-indigo-500' : 'group-hover:text-indigo-500'}`} />
                                            <span className={`font-medium text-xs transition-opacity duration-300 whitespace-nowrap ${isCollapsed ? 'lg:opacity-0 lg:w-0 lg:overflow-hidden' : 'opacity-100'}`}>
                                                {item.name}
                                            </span>
                                        </div>
                                        {!isCollapsed && (
                                        ...
                                        <div className="pl-8 pr-2 py-0.5 space-y-[2px] relative before:absolute before:left-4 before:top-0 before:bottom-0 before:w-px before:bg-slate-200 dark:before:bg-slate-800">
                                            {item.children?.map((child) => (
                                                <NavLink
                                                    ...
                                                    to={child.href}
                                                    className={({ isActive }) =>
                                                        `block px-2 py-1.5 rounded-[5px] text-[11px] font-medium transition-all duration-200 relative truncate ${isActive
                        ...
                        return (
                            <NavLink
                                ...
                                to={item.href!}
                                className={({ isActive }) =>
                                    `flex items-center gap-2 px-2 py-1.5 rounded-[5px] transition-all duration-200 group ${isActive
                            ...
                            >
                                {({ isActive }) => (
                                    <>
                                        <Icon size={16} className={isActive ? '' : 'group-hover:text-indigo-500 transition-colors'} />
                                        <span className={`text-xs transition-opacity duration-300 whitespace-nowrap ${isCollapsed ? 'lg:opacity-0 lg:w-0 lg:overflow-hidden' : 'opacity-100'}`}>
                                            {item.name}
                                        </span>
                                    </>
                                )}
