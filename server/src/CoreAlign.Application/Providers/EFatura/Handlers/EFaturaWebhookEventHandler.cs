...
namespace CoreAlign.Application.Providers.EFatura.Handlers;

public sealed class EFaturaWebhookEventHandler : IOutboxMessageHandler
{
    public const string MessageTypeKey = "provider.efatura.webhook.replay";

    public string MessageType => MessageTypeKey;

    private readonly IProviderWebhookInboxRepository _inboxRepository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ILogger<EFaturaWebhookEventHandler> _logger;

    public EFaturaWebhookEventHandler(
        IProviderWebhookInboxRepository inboxRepository,
        IUnitOfWork unitOfWork,
        ILogger<EFaturaWebhookEventHandler> logger)
    {
    ...
    public async Task<OutboxHandlerResult> HandleAsync(string payloadJson, CancellationToken cancellationToken)
    {
        ...
        EFaturaWebhookReplayEnvelope? envelope;
        try
        {
            envelope = JsonSerializer.Deserialize<EFaturaWebhookReplayEnvelope>(
                payloadJson,
        ...
        try
        {
            ...
            _logger.LogInformation(
                "EFatura webhook {InboxId} replayed for provider {Provider} (event {Event}).",
        ...
        catch (Exception ex)
        {
            ...
            _inboxRepository.Update(entry);
            await _unitOfWork.SaveChangesAsync(cancellationToken);
            _logger.LogError(ex, "Failed to replay EFatura webhook {InboxId}.", entry.Id);
            return OutboxHandlerResult.Failed(ex.Message);
        }
    }

    public sealed record EFaturaWebhookReplayEnvelope(
        Guid InboxId,
        string? ProviderName,
        string? Category,
        string? EventType,
