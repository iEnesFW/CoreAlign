...
public sealed record AcknowledgeNotificationMessageCommand(Guid MessageId, string? Note, Guid CurrentUserId)
    : IRequest<Unit>, ITransactionalRequest;

public sealed class AcknowledgeNotificationMessageHandler
    : IRequestHandler<AcknowledgeNotificationMessageCommand, Unit>
{
    private readonly ITenantContext _tenantContext;
    private readonly INotificationMessageRepository _messages;

    public AcknowledgeNotificationMessageHandler(
    ...
    public async Task<Unit> Handle(AcknowledgeNotificationMessageCommand request, CancellationToken cancellationToken)
    {
        ...
        message.Acknowledge(request.Note, request.CurrentUserId, DateTime.UtcNow);
        return Unit.Value;
    }
}
